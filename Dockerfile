FROM --platform=$BUILDPLATFORM eclipse-temurin:21.0.1_12-jre-jammy AS builder
...
WORKDIR /app
COPY --from=builder --chown=appuser:appgroup /app/build/libs/hmpps-staff-lookup-service*.jar /app/app.jar
